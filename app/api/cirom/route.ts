...
// Sonuç yalnızca istek sahibinin adıyla girilmiş siparişleri içerir;
// başka bir çalışanın rakamı sorgulanamaz (parametre yok, ad oturumdan gelir).
export async function GET(req: NextRequest) {
  ...
  try {
    const data = await memo(`cirom:${user.username}:${ayKey || "bu-ay"}`, 30_000, () =>
      computePersonalSales(user.name, ayKey || undefined)
    );

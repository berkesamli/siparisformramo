...
// Paspartu türüne göre doku (dış katman)
function matBackground(price: number, code: string, hex: string): string {
...
function mat2Background(price: number, code: string, hex: string): string {
  const c = (code || "").toUpperCase();
  if (price === 1700) {
    if (c === "W232")
...
export default function FramePreview(p: FramePreviewProps) {
  ...
  const calc = useMemo(() => {
    ...
    p.matPrice, p.mountingMM, p.frameImg, p.artImageUrl,
  ]);

  // ---- Stiller ----
  const outerMatBg = matBackground(p.matPrice, p.matColorCode, p.matColorHex);
  const innerMatBg = mat2Background(p.innerMatPrice, p.innerColorCode, p.innerColorHex);

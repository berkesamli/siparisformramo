...
// Bayi online çerçeve sihirbazı — Olga perakende sihirbazının bayi uyarlaması.
// Fiyatlar bayinin kendi ayarlarından (DealerPricing) gelir; çerçeve metre
// fiyatı sunucuda bayi çarpanıyla hesaplanır.
...
import { PASPARTU_COLORS } from "@/data/paspartu-colors";
...
interface WizardItem {
  ...
  laborCost: number;
  itemTotal: number;
  // bilgi: toptan liste maliyeti (çerçeve) — kâr göstergesi için
  frameListCost: number;
}
...
export interface WizardDealer {
  name: string;
  phone: string;
  website?: string;
}

export default function BayiWizard({
  dealer,
  pricing,
  autoRate,
  canOrder,
  ...
  canOrder: { ok: boolean; reason?: string };
}) {
  const MATS = pricing.mats;
  const INNER_MATS = useMemo(() => pricing.mats.filter((m) => m.code !== "-"), [pricing.mats]);
  const GLASSES = pricing.glasses;
  const PRINTS = pricing.prints;
  ...
  // Kur: bayi manuel kur tanımladıysa o, yoksa TCMB günlük kuru
  const initialRate =
    pricing.usdRateMode === "manual" && pricing.usdRate > 0
      ? pricing.usdRate
      : autoRate && autoRate > 0
        ? autoRate
        : pricing.usdRate > 0
          ? pricing.usdRate
          : 0;
  const [usdRate, setUsdRate] = useState(initialRate > 0 ? String(initialRate) : "");
  const rateSource =
    pricing.usdRateMode === "manual" && pricing.usdRate > 0
      ? "Ayarlardaki sabit kur"
      : autoRate && autoRate > 0
        ? "TCMB günlük kuru"
        : "";
  ...
  const [zeminMat, setZeminMat] = useState<MatType>(INNER_MATS[0]);
  ...
  const [print, setPrint] = useState<PrintType>(PRINTS[0]);
  ...
  const [success, setSuccess] = useState<{ id: string; d: string; trackUrl: string; emailSent: boolean } | null>(null);
  const [error, setError] = useState("");

  const hasMat = mat.code !== "-";

  // Seri kodu → bayinin metre fiyatı (sunucudan)
  ...
  useEffect(() => {
    ...
    lookupTimer.current = setTimeout(() => {
      fetch(`/api/fiyat/cerceve?code=${encodeURIComponent(code)}&rate=${rate}`)
  ...
  const costs = useMemo(
    () =>
      computeCosts({
        ...
        laborTL: pricing.laborTL,
      }),
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [
      wMM, hMM, edges.top, edges.right, edges.bottom, edges.left, framePriceTL,
      mat, hasMat, doubleMat, innerMat, zeminEnabled, zeminMat, glass, print, usdRate, pricing.laborTL,
    ]
  );

  // Çerçevenin toptan liste maliyeti (bilgi) — manuel fiyatta bilinmez
  const frameListCost = useMemo(() => {
    if (parseFloat(manualPrice) > 0 || !lookup?.found) return 0;
    const perim = (2 * (wMM + edges.left + edges.right + hMM + edges.top + edges.bottom)) / 1000 + 0.3;
    return perim * lookup.costTlPerM;
  }, [manualPrice, lookup, wMM, hMM, edges.left, edges.right, edges.top, edges.bottom]);
  ...
  const captureItem = useCallback((): WizardItem => {
    const fc = kurus(costs.frameCost);
    const mc = kurus(costs.matCost);
    const gc = kurus(costs.glassCost);
    const pc = kurus(costs.printCost);
    const lc = kurus(costs.laborCost);
    return {
      ...
      zeminEnabled: zeminEnabled && hasMat,
      ...
      printCost: pc,
      laborCost: lc,
      itemTotal: kurus(fc + mc + gc + pc + lc),
      frameListCost: kurus(frameListCost),
    };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [
    artWidth, artHeight, wUnit, hUnit, fullFrameCode, framePriceTL, manualPrice, mat, hasMat,
    outerColor, doubleMat, innerMat, innerColor, altMontaj, zeminEnabled, zeminMat, zeminColor,
    edges.top, edges.right, edges.bottom, edges.left, glass, print, costs, frameListCost,
  ]);
  ...
  function resetProduct() {
    ...
    setDoubleMat(false);
    setOuterColor({ code: "", hex: "" });
    setInnerMat(INNER_MATS[0]);
    setInnerColor({ code: "", hex: "" });
    setAltMontaj("5");
    setZeminEnabled(false);
    setZeminMat(INNER_MATS[0]);
  ...
  function validateStep(s: number): boolean {
    ...
    if (s === 2 && framePriceTL <= 0) {
      setError("Çerçeve fiyatı yok — geçerli bir seri kodu ve USD kuru girin veya manuel metre fiyatı yazın.");
      return false;
    }
  ...
  async function submitOrder() {
    if (!canOrder.ok) {
      setError(canOrder.reason || "Sipariş oluşturulamıyor.");
      return;
    }
    if (!currentCounts) {
      setError("Lütfen eser ölçülerini girin.");
    ...
    try {
      ...
      setSuccess({ id: d.orderId, d: d.dateKey || "", trackUrl: d.trackUrl || "", emailSent: Boolean(d.emailSent) });
    } catch (e: any) {
  ...
  const brandLine = `${dealer.name} | ${dealer.phone}${dealer.website ? " | " + dealer.website : ""}`;
  ...
  function sendWhatsAppQuote() {
    ...
    const lines = [`*${dealer.name} — Fiyat Teklifi*`, ""];
    items.forEach((it, i) => {
  ...
  function sendWhatsAppTracking() {
    ...
    const phone = normalizePhoneWa(customerPhone);
    const text = [
      `*${dealer.name} — Sipariş ${success.id}*`,
      `Sayın ${customerName.trim()}, siparişiniz alındı.`,
      deliveryDate ? `Tahmini teslim: ${deliveryDate}` : "",
      `Toplam: ${fmt(grandTotal)} TL`,
      "",
      success.trackUrl ? `Sipariş takibi: ${success.trackUrl}` : "",
      brandLine,
    ]
      .filter((l) => l !== "")
      .join("\n");
  ...
  function ColorPalette({
    ...
    return (
      <div className="rw-palette">
        ...
        {selected.code && <span className="rw-color-label">Seçili: {selected.code}</span>}
      </div>
    );
  }
  ...
  if (success) {
    ...
    return (
      <div className="card" style={{ maxWidth: 600, margin: "40px auto", textAlign: "center" }}>
        ...
        {success.emailSent && (
          <p style={{ fontSize: 13, color: "var(--success)" }}>✓ Müşteriye e-posta ile sipariş özeti ve PDF gönderildi.</p>
        )}
        <div style={{ display: "flex", gap: 10, justifyContent: "center", marginTop: 16, flexWrap: "wrap" }}>
          <button className="btn wa" onClick={sendWhatsAppTracking}>
            📲 Müşteriye WhatsApp Gönder
          </button>
          {success.d && (
            <>
              <a href={`/api/siparisler/pdf?${q}`} className="btn">
                ⬇ Üretim PDF
              </a>
              <Link href={`/panel/siparisler/detay?${q}`} className="btn secondary">
                🖨️ Fişi Gör
              </Link>
            </>
          )}
        ...
        {success.trackUrl && (
          <p style={{ fontSize: 12.5, color: "var(--muted)", marginTop: 14, wordBreak: "break-all" }}>
            Müşteri takip linki: <a href={success.trackUrl} target="_blank" rel="noreferrer">{success.trackUrl}</a>
          </p>
        )}
        <p style={{ fontSize: 12.5, color: "var(--muted)", marginTop: 12 }}>
          <Link href="/panel/siparisler">Siparişler listesine git →</Link>
        </p>
      </div>
    );
  }
      ...
      <div>
        {!canOrder.ok && (
          <div className="notice err" style={{ marginBottom: 14 }}>
            {canOrder.reason} Fiyat hesaplayabilir ve WhatsApp teklifi gönderebilirsiniz; sipariş kaydı kapalıdır.
          </div>
        )}
          ...
          <div className="card">
            ...
            <div style={{ marginTop: 16 }}>
              ...
              <div className="rw-presets">
                {SIZE_PRESETS.map((s) => {
                  const active =
                    wUnit === "cm" && hUnit === "cm" && parseFloat(artWidth) === s.w && parseFloat(artHeight) === s.h;
                  return (
                    <button
                      key={s.label}
                      type="button"
        ...
        {step === 2 && (
          <div className="card">
            ...
            <div className="rw-grid2">
              <div>
                <label>Seri Kodu (Olga kataloğu)</label>
                <input value={seriesCode} onChange={(e) => setSeriesCode(e.target.value.toUpperCase())} placeholder="örn. GC065" />
                <span
                  style={{
                    fontSize: 12.5,
                    fontWeight: 600,
                    color: looking ? "var(--muted)" : lookup?.found ? "var(--success)" : seriesCode ? "var(--error)" : "var(--muted)",
                  }}
                >
                  {looking
                    ? "Aranıyor..."
                    : lookup?.found
                      ? `✓ ${lookup.resolvedCode} — ₺${fmt(lookup.tlPerM)}/metre (satış)`
                      : seriesCode
                ...
                {lookup?.found && (
                  <span style={{ display: "block", fontSize: 12, color: "var(--muted)" }}>
                    Toptan liste: ₺{fmt(lookup.costTlPerM)}/m · çarpan ×{pricing.frameFactor}
                  </span>
                )}
              </div>
              <div>
                <label>Renk Kodu (opsiyonel)</label>
                <input value={colorCode} onChange={(e) => setColorCode(e.target.value.toUpperCase())} placeholder="örn. 1473" />
                {frameImg && (
              ...
              <div>
                ...
                <input type="number" step="0.01" value={usdRate} onChange={(e) => setUsdRate(e.target.value)} placeholder="örn. 47.50" />
                {rateSource && String(initialRate) === usdRate && (
                  <span style={{ fontSize: 12, color: "var(--success)" }}>✓ {rateSource}</span>
                )}
              </div>
              <div>
                <label>Manuel Metre Fiyatı (₺/m)</label>
                <input type="number" min="0" step="0.01" value={manualPrice} onChange={(e) => setManualPrice(e.target.value)} placeholder="Kod yoksa elle girin" />
              </div>
            </div>
            {framePriceTL > 0 && (
        ...
        {step === 3 && (
          <div className="card">
            ...
            <div className="rw-options">
              {MATS.map((m) => (
                <button key={m.code} type="button" className={`rw-option ${mat.code === m.code ? "sel" : ""}`} onClick={() => selectMat(m)}>
                  <span className="rw-option-icon">{m.icon}</span>
                  <span className="rw-option-name">{m.name}</span>
                  {m.code !== "-" && <span className="rw-option-desc">₺{fmt(m.price)}/m²</span>}
                </button>
              ))}
            </div>
                  ...
                  <>
                    <div style={{ marginTop: 16 }}>
                      ...
                      <div className="rw-minis">
                        {INNER_MATS.map((m) => (
                          <button key={m.code} type="button" className={`rw-mini ${innerMat.code === m.code ? "sel" : ""}`} onClick={() => { setInnerMat(m); setInnerColor({ code: "", hex: "" }); }}>
                            {m.name}
                          </button>
                        ))}
                      </div>
                ...
                <div style={{ marginTop: 18 }}>
                  ...
                  {zeminEnabled && (
                    <div style={{ marginTop: 10 }}>
                      <div className="rw-minis">
                        {INNER_MATS.map((m) => (
                          <button key={m.code} type="button" className={`rw-mini ${zeminMat.code === m.code ? "sel" : ""}`} onClick={() => { setZeminMat(m); setZeminColor({ code: "", hex: "" }); }}>
                            {m.name}
                          </button>
                        ))}
                      </div>
        ...
        {step === 4 && (
          <div className="card">
            ...
            <div className="rw-options">
              {GLASSES.map((g) => (
                <button key={g.name} type="button" className={`rw-option ${glass.name === g.name ? "sel" : ""}`} onClick={() => setGlass(g)}>
                  <span className="rw-option-icon">{g.icon}</span>
                  <span className="rw-option-name">{g.name}</span>
                  <span className="rw-option-desc">{g.price > 0 ? `₺${fmt(g.price)}/m² · ` : ""}{g.desc}</span>
                </button>
              ))}
            </div>
          </div>
        ...
        {step === 5 && (
          <div className="card">
            ...
            <p className="subtitle" style={{ marginTop: 0 }}>Baskı, eserin kendi ölçüsü üzerinden hesaplanır.</p>
            <div className="rw-options">
              {PRINTS.map((p) => (
                <button key={p.name} type="button" className={`rw-option ${print.name === p.name ? "sel" : ""}`} onClick={() => setPrint(p)}>
                  <span className="rw-option-icon">{p.icon}</span>
                  <span className="rw-option-name">{p.name}</span>
                  <span className="rw-option-desc">{p.usdPerM2 > 0 ? `$${p.usdPerM2}/m² · ` : ""}{p.desc}</span>
                </button>
              ))}
            </div>
          </div>
        ...
        {step === 6 && (
          <div className="card">
            ...
            <table style={{ marginBottom: 14 }}>
              <tbody>
                ...
                <tr>
                  ...
                  <td>
                    {mat.name}
                    {hasMat && outerColor.code && ` — ${outerColor.code}`}
                ...
                {cart.map((it, i) => (
                  <div className="rw-cart-item" key={i}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                      ...
                      <button className="btn small danger" onClick={() => setCart(cart.filter((_, j) => j !== i))}>Sil</button>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="rw-totals">
              ...
              {costs.laborCost > 0 && <div><span>İşçilik</span><span>₺{fmt(costs.laborCost)}</span></div>}
              {cart.length > 0 && (
                <>
            ...
            {frameListCost > 0 && currentCounts && (
              <p style={{ fontSize: 12.5, color: "var(--muted)", marginTop: 8 }}>
                Bilgi (müşteriye gösterilmez): bu ürünün çerçeve profili toptan liste maliyeti ≈ ₺{fmt(frameListCost)} —
                çerçeve kalemindeki brüt fark ₺{fmt(costs.frameCost - frameListCost)}.
              </p>
            )}
            ...
            <div style={{ display: "flex", gap: 10, marginTop: 20, flexWrap: "wrap" }}>
              <button className="btn secondary" onClick={addToCart}>➕ Sepete Ekle & Yeni Ürün</button>
            </div>
          </div>
        )}
        ...
        {step === 7 && (
          <div className="card">
            ...
            <div className="rw-grid2">
              ...
              <div>
                <label>E-posta (PDF gönderimi için)</label>
              ...
              <div style={{ gridColumn: "1 / -1" }}>
                ...
                <input value={customerAddress} onChange={(e) => setCustomerAddress(e.target.value)} placeholder="Mah. Cad. No:X D:Y  İLÇE / İL" />
              </div>
            </div>
            ...
            <div style={{ display: "flex", gap: 10, marginTop: 20, flexWrap: "wrap" }}>
              <button className="btn wa" onClick={sendWhatsAppQuote}>📲 WhatsApp Teklif Gönder</button>
              <button className="btn" disabled={submitting || !canOrder.ok} onClick={submitOrder}>
                {submitting ? "Kaydediliyor..." : "✅ Siparişi Kaydet"}
              </button>
            </div>
            <p style={{ fontSize: 12, color: "var(--muted)", marginTop: 10 }}>Bayi: {dealer.name}</p>
          </div>
        )}
        ...
        <div style={{ display: "flex", justifyContent: "space-between", marginTop: 18 }}>
          <button className="btn secondary" disabled={step === 1} onClick={() => setStep(step - 1)}>← Geri</button>
          {step < 7 && <button className="btn" onClick={next}>İleri →</button>}
        </div>
      </div>
          ...
          {currentCounts && costs.itemTotal > 0 && (
            <div className="fp-cost">
              <div className="fp-cost-bar">
                ...
                <span className="seg print" style={{ width: `${((costs.printCost + costs.laborCost) / costs.itemTotal) * 100}%` }} />
              </div>
              <div className="fp-cost-legend">
                ...
                {costs.laborCost > 0 && <span><i className="dot print" /> İşçilik <strong>₺{fmt(costs.laborCost)}</strong></span>}
              </div>
            </div>
          )}

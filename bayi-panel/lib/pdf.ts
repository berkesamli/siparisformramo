// Bayi sipariş üretim PDF'i — Olga perakende üretim PDF'inin bayi markalı uyarlaması.
// Başlık ve alt bilgide bayinin firma adı/telefonu yazılır. Tek sayfa A4: koyu altın bantlar, ölçekli
...
import type { DealerBrand } from "./notify";
...
function geom(it: OrderItem): ItemGeom {
...
function matColor(it: OrderItem, inner = false): string {
...
export function generateOrderPdf(o: SavedOrder, brand: DealerBrand): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    ...
    doc.text(brand.name.slice(0, 32), mm(15), mm(7), { width: mm(110), height: mm(10), ellipsis: true });
    ...
    doc.text(brand.name.slice(0, 40), mm(15), H - mm(13));
    doc.font(FONT).fontSize(8).fillColor("#888888");
    doc.text(brand.phone || "", 0, H - mm(13), { width: W, align: "center" });
    doc.text(brand.website || "Olga Çerçeve yetkili bayisi", W - mm(95), H - mm(13), { width: mm(80), align: "right" });
